...
// .wrangler/tmp/bundle-ojWg6Y/checked-fetch.js
...
// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/utils/encode.js
...
// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/utils/jwt/jwa.js
...
// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/helper/adapter/index.js
var knownUserAgents = {
  deno: "Deno",
  bun: "Bun",
...
// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/utils/jwt/types.js
...
// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/utils/jwt/utf8.js
var utf8Encoder = new TextEncoder();
var utf8Decoder = new TextDecoder();

// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/utils/jwt/jws.js
...
// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/utils/jwt/jwt.js
...
// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/utils/jwt/index.js
var Jwt = { sign, verify, decode, verifyWithJwks };

// node_modules/.pnpm/hono@4.9.10/node_modules/hono/dist/middleware/jwt/jwt.js
...
__name(getUserIdFromPayload, "getUserIdFromPayload");
...
var profilesRoutes = app2;

// src/routes/avoidance-areas.ts
var app3 = new Hono2();
app3.get("/", async (c) => {
  try {
...
app4.get("/", async (c) => {
  try {
    ...
    const parsedPois = pois.results.map((poi) => ({
      ...poi,
...
app4.get("/nearby", async (c) => {
  try {
    const { lat, lng, radius = 1e3, type } = c.req.query();
    if (!lat || !lng) {
    ...
    const nearbyPois = pois.results.filter((poi) => {
    ...
    const parsedPois = nearbyPois.map((poi) => ({
      ...poi,
      metadata: JSON.parse(poi.metadata),
      location_geojson: JSON.parse(poi.location_json)
    }));
...
app4.post("/", async (c) => {
  try {
    ...
    const parsedPoi = {
      ...newPoi,
      metadata: JSON.parse(newPoi.metadata),
      location_geojson: JSON.parse(newPoi.location_json)
    };
...
app4.put("/:id", async (c) => {
  try {
    ...
    const parsedPoi = {
      ...updatedPoi,
      metadata: JSON.parse(updatedPoi.metadata),
      location_geojson: JSON.parse(updatedPoi.location_json)
    };
  ...
  try {
    ...
    if (existingPreferences) {
      result = await c.env.DB.prepare(
        ...
        avoid_areas !== void 0 ? avoid_areas : null,
        gradient_tolerance !== void 0 ? gradient_tolerance : null,
        userId
      ).run();
    } else {
      result = await c.env.DB.prepare(
        ...
        userId,
        avoid_areas !== void 0 ? avoid_areas : true,
        gradient_tolerance !== void 0 ? gradient_tolerance : 0.05
      ).run();
    }
    if (!result.success) {
...
app5.get("/locations", async (c) => {
  try {
    ...
    const parsedLocations = locations.results.map((location) => ({
      ...location,
...
app5.post("/locations", async (c) => {
  try {
    ...
    const parsedLocation = {
      ...newLocation,
      location: JSON.parse(newLocation.location_json)
    };
...
app5.put("/locations/:id", async (c) => {
  try {
    ...
    const parsedLocation = {
      ...updatedLocation,
      location: JSON.parse(updatedLocation.location_json)
    };
...
app5.delete("/locations/:id", async (c) => {
  try {
    const payload = c.get("jwtPayload");
    const userId = getUserIdFromPayload(payload);
    if (!userId) {
...
// src/utils/sync.ts
async function syncAccessibleEntrances(db, kmlUrl) {
  try {
...
__name(syncAccessibleEntrances, "syncAccessibleEntrances");
async function parseKMLToPOIs(kmlText) {
  const pois = [];
  try {
...
__name(parseKMLToPOIs, "parseKMLToPOIs");
async function parsePlacemark(placemarkContent) {
  try {
...
__name(parsePlacemark, "parsePlacemark");

// src/routes/sync.ts
var app6 = new Hono2();
app6.post("/pois", async (c) => {
  try {
    const accessibleEntrancesKmlUrl = "https://www.google.com/maps/d/kml?forcekml=1&mid=1B_X9WRe0kkTlPbfYpmOQz7pHSQs";
...
var jwtMiddleware = jwt({
  secret: /* @__PURE__ */ __name((c) => c.env.JWT_SECRET || "dev-secret-key-change-in-production", "secret")
});
app7.get("/", (c) => {
  return c.json({
...
app7.route("/auth", authRoutes);
app7.route("/sync", syncRoutes);
app7.use("/avoidance-areas", async (c, next) => {
  if (c.req.method !== "GET") {
    return jwtMiddleware(c, next);
  }
  return next();
});
app7.use("/pois", async (c, next) => {
  if (c.req.method !== "GET") {
    return jwtMiddleware(c, next);
  }
  return next();
});
app7.route("/avoidance-areas", avoidanceAreasRoutes);
app7.route("/pois", poisRoutes);
app7.use("/profiles/*", jwtMiddleware);
app7.use("/user/*", jwtMiddleware);
app7.route("/profiles", profilesRoutes);
app7.route("/user", userRoutes);
app7.onError((err, c) => {
...
// node_modules/.pnpm/wrangler@4.42.0_@cloudflare+workers-types@4.20251004.0/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// node_modules/.pnpm/wrangler@4.42.0_@cloudflare+workers-types@4.20251004.0/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-ojWg6Y/middleware-insertion-facade.js
...
// node_modules/.pnpm/wrangler@4.42.0_@cloudflare+workers-types@4.20251004.0/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-ojWg6Y/middleware-loader.entry.ts
